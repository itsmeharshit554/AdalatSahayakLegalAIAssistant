...
def index():
    ...
    if request.method == 'POST':
        ...
        if 'user_text' in request.form and request.form['user_text'].strip():
            user_text = request.form['user_text'][:7000]  # Limit input size
...
def as_summary(txt):
    try:
        load_summarizer()
        summary = summarizer(txt, max_length=150, min_length=60, do_sample=False)
        return summary[0]['summary_text']
    except Exception as e:
        return f"Error during summarization: Error due to memory exceeding limit on Render Backend. (limit 512MB)"
